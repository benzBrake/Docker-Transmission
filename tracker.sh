...
auth=${TR_USERNAME}:${TR_PASSWORD}
host=localhost
add_trackers() {
    torrent_hash=$1
    id=$2
    trackerslist=/tmp/trackers.txt
    for base_url in ${TR_TRACKERS_UPDATE_URL:-https://raw.githubusercontent.com/ngosang/trackerslist/master/trackers_all.txt}; do
